...
$hasilob = mysql_query("SELECT nama_obat FROM obat");
...
$hasil3 = mysql_query("SELECT nama_poli FROM poli_lab WHERE kode_dokter IN (SELECT kode_dokter FROM tgl_masuk WHERE kode_pasien='$kode_pasien')");
...
$obat = mysql_fetch_array($hasilob);

$rujukan = mysql_fetch_array($hasil22);
...
<input name="kode_ruangan" type="hidden" value="<?php echo $ruangan[kode_ruangan]?>"/>
  ...
  <tr>
    <td>Poli Lab</td>
    <td>:</td>
    <td><?php $poli = mysql_fetch_array($hasil3); echo $poli['nama_poli']; ?></td>
  </tr>
  <tr>
